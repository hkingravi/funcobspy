...
def time_varying_uncertainty(weights_star, t, scheme):
    ...
    if scheme == "smooth1":
        ...
        weights_star[2] = max(min(0.1*np.cos(1.1 * t) + weights_star[2], 2), -2)
        weights_star[3] = 0.3 * (np.sin(weights_star[4]) + weights_star[3] * max(min(0.1 * np.cos(1.1 * t) +
                                                                                     0.3 * np.sin(25 * t), 2), -2))
        weights_star[4] = 0.8 * np.cos(2.2 * t) + max(min(0.1 * np.cos(1.1*t)*weights_star[0] + weights_star[4], 2), -2)
    elif scheme == "smooth4":
