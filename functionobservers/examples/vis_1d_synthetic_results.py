...
def animate_local_pred_data(ii):
    ...
    pred_line.set_ydata(np.squeeze(pred_local_plot_vals[:, :, ii]))
...
if debug_flag:
    loadfile_local = os.path.join(results_dir, "synt_1d_timeseries_dnn_local_results_" + f_scheme + "_debug.pkl")
...
if show_linear_pred:
    if debug_flag:
        ...
        loadfile_local = os.path.join(results_dir, "synt_1d_timeseries_dnn_local_results_" + f_scheme + ".pkl")
    data_local_dict = pickle.load(open(loadfile_local, "rb"))
    pred_local_plot_vals = data_local_dict['pred_plot_vals']

if show_linear_pred:
    if debug_flag:
        loadfile_linear = os.path.join(results_dir, "synt_1d_timeseries_dnn_linear_results_" + f_scheme + "_debug.pkl")
    else:
...
if show_global_pred:
    if debug_flag:
        loadfile_global = os.path.join(results_dir, "synt_1d_timeseries_dnn_global_results_" + f_scheme + "_debug.pkl")
    else:
